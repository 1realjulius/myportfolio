...
import { FileText, Package, X } from "react-feather";
...
const PricesAndPackages = () => {
  ...
  return (
    <div className="flex">
      ...
      <div
        onClick={openModal}
        className="flex flex-row items-center justify-between w-full dark:border-2 p-2 rounded-md bg-blue-200 dark:bg-inherit dark:border-[#24292f]"
      >
        <span className="flex dark:text-blue-600">
            
        <FileText className="flex text-gray-500 dark:text-blue-600 animate-headShake" size={24} />
            View My Prices and Packages</span>
        <Package className="flex text-gray-500 dark:text-blue-600 animate-headShake" size={24} />
      </div>
    </div>
  );
};
